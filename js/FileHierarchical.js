 ...
 */
var outputData = {};
outputData.reactions = {};
outputData.reactions.name = "reaction";
outputData.reactions.children = [];
outputData.biomolecules = {};
outputData.biomolecules.name = "biomolecules";
outputData.biomolecules.children = [];
outputData.links = [];
function File(objects) {
    ...
    this.flag = false;
    function animate() {
        if (outputData.links.length) {
            $.ajax({
                url: 'fileHierarchical.php',
                type: "POST",  // type should be POST
                data: {
                    fileName: "./HierarchicalData/"+objects[processedIdex].dbId+".json",
...
//                        alert("Sorry, your work can not be saved, Please try it again!");
            ...
            //dbId = objects[processedIdex].dbId;
            _this.processData(objects[processedIdex].dbId);  //109581
//            _this.processData("109581");
        ...
        if (!this.flag) {
            ...
            this.removeUnwantedData();
            {
                for(var i=0; i<Data.compartments.length; ++i)
                {
                    //Reactions
                    var compartReaction = {};
                    compartReaction.name = Data.compartments[i].name;
                    compartReaction.children= [];
                    for(var j=0; j<Data.compartments[i].associations.length; ++j)
                    {
                        var association = {};
                        association.name = Data.compartments[i].associations[j].name;
                        association.id = Data.compartments[i].associations[j].id;
                        association.type = "Association";
                        association.parentCompartmentName=Data.compartments[i].associations[j].parentCompartmentName;
                        compartReaction.children.push(association);
                    }
                    for(var j=0; j<Data.compartments[i].dissociations.length; ++j)
                    {
                        var dissociations = {};
                        dissociations.name = Data.compartments[i].dissociations[j].name;
                        dissociations.id = Data.compartments[i].dissociations[j].id;
                        dissociations.type = "Dissociations";
                        dissociations.parentCompartmentName=Data.compartments[i].dissociations[j].parentCompartmentName;
                        compartReaction.children.push(dissociations);
                    }
                    for(var j=0; j<Data.compartments[i].transitions.length; ++j)
                    {
                        var transitions = {};
                        transitions.name = Data.compartments[i].transitions[j].name;
                        transitions.id = Data.compartments[i].transitions[j].id;
                        transitions.type = "Transitions";
                        transitions.parentCompartmentName=Data.compartments[i].transitions[j].parentCompartmentName;
                        compartReaction.children.push(transitions);
                    }
                    if(compartReaction.children.length)
                        outputData.reactions.children.push(compartReaction);
                    ...
                    {
                        var _complex = {};
                        _complex.name = Data.compartments[i].complexs[j].name;
                        _complex.id = Data.compartments[i].complexs[j].id;
                        _complex.reactomeDbId = Data.compartments[i].complexs[j].reactomeDbId;
                        complex.children.push(_complex);
                    }
                    if(complex.children.length)
                    {
                       for(var k=0; k<outputData.biomolecules.children.length;++k)
                       {
                            if(outputData.biomolecules.children[k].name == "Complex")
                            {
                                break;
                            }
                       }
                        if(k>=outputData.biomolecules.children.length)
                            outputData.biomolecules.children.push(complex);
                        else
                        {
                            for(var kk=0; kk<complex.children.length; ++kk)
                            outputData.biomolecules.children[k].children.push(complex.children[kk]);
                        }
                    }
                    ...
                    {
                        var _protein = {};
                        _protein.name = Data.compartments[i].proteins[j].name;
                        _protein.id = Data.compartments[i].proteins[j].id;
                        _protein.reactomeDbId = Data.compartments[i].proteins[j].reactomeDbId;
                        protein.children.push(_protein);
                    }
                    if(protein.children.length)
                        {
                            for(var k=0; k<outputData.biomolecules.children.length;++k)
                            {
                                if(outputData.biomolecules.children[k].name == "Protein")
                                {
                                    break;
                                }
                            }
                            if(k>=outputData.biomolecules.children.length)
                                outputData.biomolecules.children.push(protein);
                            else
                            {
                                for(var kk=0; kk<protein.children.length; ++kk)
                                    outputData.biomolecules.children[k].children.push(protein.children[kk]);
                            }
                        }
                    ...
                    {
                        var _dna = {};
                        _dna.name = Data.compartments[i].dnas[j].name;
                        _dna.id = Data.compartments[i].dnas[j].id;
                        _dna.reactomeDbId = Data.compartments[i].dnas[j].reactomeDbId;
                        dna.children.push(_dna);
                    }
                    if(dna.children.length)
                    {
                        for(var k=0; k<outputData.biomolecules.children.length;++k)
                        {
                            if(outputData.biomolecules.children[k].name == "Dna")
                            {
                                break;
                            }
                        }
                        if(k>=outputData.biomolecules.children.length)
                            outputData.biomolecules.children.push(dna);
                        else
                        {
                            for(var kk=0; kk<dna.children.length; ++kk)
                                outputData.biomolecules.children[k].children.push(dna.children[kk]);
                        }
                    }
                    ...
                    {
                        var _rna = {};
                        _rna.name = Data.compartments[i].rnas[j].name;
                        _rna.id = Data.compartments[i].rnas[j].id;
                        _rna.reactomeDbId = Data.compartments[i].rnas[j].reactomeDbId;
                        rna.children.push(_rna);
                    }
                    if(rna.children.length)
                    {
                        for(var k=0; k<outputData.biomolecules.children.length;++k)
                        {
                            if(outputData.biomolecules.children[k].name == "Rna")
                            {
                                break;
                            }
                        }
                        if(k>=outputData.biomolecules.children.length)
                            outputData.biomolecules.children.push(rna);
                        else
                        {
                            for(var kk=0; kk<rna.children.length; ++kk)
                                outputData.biomolecules.children[k].children.push(rna.children[kk]);
                        }
                    }
                    ...
                    {
                        var _entity = {};
                        _entity.name = Data.compartments[i].entitys[j].name;
                        _entity.id = Data.compartments[i].entitys[j].id;
                        _entity.reactomeDbId = Data.compartments[i].entitys[j].reactomeDbId;
                        entity.children.push(_entity);
                    }
                    if(entity.children.length)
                    {
                        for(var k=0; k<outputData.biomolecules.children.length;++k)
                        {
                            if(outputData.biomolecules.children[k].name == "Entity")
                            {
                                break;
                            }
                        }
                        if(k>=outputData.biomolecules.children.length)
                            outputData.biomolecules.children.push(entity);
                        else
                        {
                            for(var kk=0; kk<entity.children.length; ++kk)
                                outputData.biomolecules.children[k].children.push(entity.children[kk]);
                        }
                    }
                    ...
                    {
                        var _molecule = {};
                        _molecule.name = Data.compartments[i].molecules[j].name;
                        _molecule.id = Data.compartments[i].molecules[j].id;
                        _molecule.reactomeDbId = Data.compartments[i].molecules[j].reactomeDbId;
                        molecule.children.push(_molecule);
                    }
                    if(molecule.children.length)
                    {
                        for(var k=0; k<outputData.biomolecules.children.length;++k)
                        {
                            if(outputData.biomolecules.children[k].name == "Molecule")
                            {
                                break;
                            }
                        }
                        if(k>=outputData.biomolecules.children.length)
                            outputData.biomolecules.children.push(molecule);
                        else
                        {
                            for(var kk=0; kk<molecule.children.length; ++kk)
                                outputData.biomolecules.children[k].children.push(molecule.children[kk]);
                        }
                    }
                }
                for(var i=0; i<Data.arrows.length; ++i)
                {
                    ...
                    link.type = "Arrow";
                    link.begin ={};
                    link.begin.id = Data.arrows[i].begin.id;
                    link.begin.type = Data.arrows[i].begin.type;

                    link.end ={};
                    link.end.id = Data.arrows[i].end.id;
                    link.end.type = Data.arrows[i].end.type;
                    outputData.links.push(link);
                }
                for(var i=0; i<Data.activations.length; ++i)
                {
                    ...
                    link.begin ={};
                    link.begin.id = Data.activations[i].begin.id;
                    link.begin.type = Data.activations[i].begin.type;

                    link.end ={};
                    link.end.id = Data.activations[i].end.id;
                    link.end.type = Data.activations[i].end.type;
                    outputData.links.push(link);
                }
                for(var i=0; i<Data.inhibitions.length; ++i)
                {
                    var link={};
                    link.type = "Inhibitions";
                    link.begin ={};
                    link.begin.id = Data.inhibitions[i].begin.id;
                    link.begin.type = Data.inhibitions[i].begin.type;

                    link.end ={};
                    link.end.id = Data.inhibitions[i].end.id;
                    link.end.type = Data.inhibitions[i].end.type;
                    outputData.links.push(link);
                }
                        ...
                        if (lines[i] !== "") {
                            ...
                            if (k >= Data.compartments.length) {
                                ...
                                if (j >= compartment.complexs.length) {
                                    ...
                                    var reactomeDbId = array[3].split(": ")[1];
                                ...
                                if (j >= currentCompartment.complexs.length) {
                                    ...
                                    var type = array[4].replace(/\d+/g, '');
                                    _this.addChild(complex, type, Id);
                                    complex.parent=currentCompartment;
                        ...
                        if (lines[i] !== "") {
                            ...
                            if (k >= Data.compartments.length) {
                                ...
                                if (j >= compartment.proteins.length) {
                                    ...
                                    var  uniProtId = array[2].split(":")[1];
                                    protein.uniProtId = uniProtId;

                                    var  reactomeDbId = array[3].split(": ")[1];
                                    protein.reactomeDbId = reactomeDbId;

                                    protein.name = array[1];
                                    protein.parent = compartment;
                                ...
                                if (j >= currentCompartment.proteins.length) {
                                    ...
                                    var  reactomeDbId = array[3].split(": ")[1];
                                    protein.reactomeDbId = reactomeDbId;
                                    protein.parent = currentCompartment;
                                ...
                                if (k >= Data.compartments.length) {
                                    ...
                                    if (type === "SmallMolecule") {
                                        ...
                                        if (j >= currentCompartment.molecules.length) {
                                            ...
                                            molecule.h = 15;
                                            molecule.name = array[1];
                                            molecule.parent = currentCompartment;
            ...
            success: function (result) {
                if (result) {
                    ...
                    for (var i = 0; i < lines.length; ++i) {
                        ...
                        if (lines[i] !== "") {
                            var array = lines[i].split("\t");
                            if (array[0].replace(/\d+/g, '') !== "BiochemicalReaction" || array[2].replace(/\d+/g, '') === "NA" || array[3].replace(/\d+/g, '') === "NA") {
                                continue;
                            }
    ...
    addArrowReaction: function (reaction) {
        if (reaction.input.length > 1 && reaction.output.length === 1)   //bind association
        {
            ...
            for (var i = 0; i < reaction.input.length; ++i) {
                ...
                if (flag) {
                    ...
                    if (compartment) {
                        ...
                        association.parent = compartment;
                ...
                for (var j = 0; j < Data.arrows.length; ++j) {
                    if (Id === Data.arrows[j].begin.id && reaction.id === Data.arrows[j].end.id) {
                        break;
                    }
                }
                if (j >= Data.arrows.length) {
                    ...
                    if(!arrow.begin||!arrow.end)
                        return;
                    association.parentCompartmentName = arrow.begin.parent.name;
                    arrow.end.flag = true;
            ...
            for (var i = 0; i < reaction.output.length; ++i) {
                ...
                for (var j = 0; j < Data.arrows.length; ++j) {
                    if (reaction.id === Data.arrows[j].begin.id && Id === Data.arrows[j].end.id) {
                        break;
                    }
                }
                if (j >= Data.arrows.length) {
                    ...
                    if(!arrow.begin||!arrow.end)
                        return;
                    association.parentCompartmentName = arrow.end.parent.name;
                    arrow.end.flag = true;
                    arrow.begin.flag = true;
                    Data.arrows.push(arrow);
                }
        ...
        else if (reaction.input.length === 1 && reaction.output.length > 1)   //dissociation
        {
            ...
            for (var i = 0; i < reaction.input.length; ++i) {
                ...
                if (flag) {
                    ...
                    if (compartment) {
                        ...
                        dissociation.parent = compartment;
                ...
                if (j >= Data.arrows.length) {
                    ...
                    if(!arrow.begin||!arrow.end)
                        return;
                    dissociation.parentCompartmentName = arrow.begin.parent.name;
                    arrow.end.flag = true;
                    arrow.begin.flag = true;
                    Data.arrows.push(arrow);
                }
            ...
            for (var i = 0; i < reaction.output.length; ++i) {
                ...
                if (j >= Data.arrows.length) {
                    ...
                    if(!arrow.begin||!arrow.end)
                        return;
                    dissociation.parentCompartmentName = arrow.end.parent.name;
                    arrow.end.flag = true;
                    arrow.begin.flag = true;
                    Data.arrows.push(arrow);
                }
            ...
            for (var i = 0; i < reaction.input.length; ++i) {
                ...
                if (j >= Data.arrows.length) {
                    ...
                    if(!arrow.begin||!arrow.end)
                        return;
                    transition.parentCompartmentName = arrow.begin.parent.name;
                    arrow.end.flag = true;
                    arrow.begin.flag = true;
                    Data.arrows.push(arrow);
                }
            ...
            for (var i = 0; i < reaction.output.length; ++i) {
                ...
                if (j >= Data.arrows.length) {
                    ...
                    if(!arrow.begin||!arrow.end)
                        return;
                    transition.parentCompartmentName = arrow.end.parent.name;
                    arrow.end.flag = true;
                    arrow.begin.flag = true;
    ...
    getCompartmentByName: function (type, id) {
        switch (type) {
            ...
            /*
             ...
             for(var i=0; i<Data11.compartments.length; ++i)
             {
             for(var j=0; j< Data11.compartments[i].dnas.length; ++j)
             {
             if(Data11.compartments[i].dnas[j].id === id)
             {
             return  Data11.compartments[i];
             }
             }
             }
             return null;  */
                        ...
                        if (lines[i] !== "") {
                            ...
                            if (array[3] === "ACTIVATION") {
                                ...
                                for (var j = 0; j < Data.activations.length; ++j) {
                                    if (begin.id === Data.activations[j].begin.id && end.id === Data.activations[j].end.id) {
                                        break;
                                    }
                                }
