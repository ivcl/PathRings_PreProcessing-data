                ...
                $.ajax({
                    ...
                    success: function (txt) {
                        ...
                        if (txt.length !== 0) {
                            ...
                            //--------------------------------------------------
                            for (var i = 0; i < hierarchObjects.length; ) {
                                var arr = [];
                                var links = [];
                                var fileName = hierarchObjects[i].name;

                                function traverse(data, level) {
                                    if (arr[level] == undefined) arr[level] = [];
                                    arr[level].push(data.name);
                                    for (var index = 0; index < data.children.length; index++) {
                                        traverse(data.children[index], level + 1);
                                    }
                                }

                                traverse(hierarchObjects[i], 0);
                                arr.splice(0, 1);
                                for (var ii = 0; ii < arr.length; ++ii) {
                                    var leftLevel = arr[ii];
                                    var levelLinks = [];
                                    for (var k = 0; k < crossTalkPathways.length; ++k)
                                    {
                                        ...
                                        for (var kk = 0; kk < names.length; ++kk) {
                                            for (var j = 0; j < leftLevel.length; ++j) {
                                                if (names[kk].replace(/^\s\s*/, '').replace(/\s\s*$/, '') == leftLevel[j].replace(/^\s\s*/, '').replace(/\s\s*$/, '')) {
                                                    tempPathways.push(leftLevel[j]);
                                                }
                                            }
                                        }
                                        ...
                                        objt.imports = [];
                                        for (var kkt = 1; kkt < tempPathways.length; ++kkt) {
                                ...
                                $.ajax({
                                    ...
                                    data: {
                                        fileName: "./crossTalkLevel/" + fileName + '.json',
