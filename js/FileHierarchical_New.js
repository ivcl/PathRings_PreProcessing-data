    ...
    processData: function (fileNum) {
        if (!this.flag) {
            ...
            this.removeUnwantedData();
            {
                ...
                {
                    var begin = Data.activations[i].begin;
                    var end = Data.activations[i].end;
                    var beginParentName = begin.parent.name.replace(/[<>:,&.'"\/\\|?*]+/g, '');  //& < > " ' xml special characters
                    var endParentName = end.parent.name.replace(/[<>:,&.'"\/\\|?*]+/g, '');  //& < > " ' xml special characters
                    var beginName = begin.name.replace(/[<>:,&.'"\/\\|?*]+/g, '');  //& < > " ' xml special characters
                    var endName = end.name.replace(/[<>:,&.'"\/\\|?*]+/g, '');  //& < > " ' xml special characters
                    var obj ={};
                    obj.name = "Pathway." + beginParentName +"."+beginName;
                    obj.import = "Pathway." + endParentName +"."+endName;
                    links.push(obj);
                }
                for(var i=0; i<Data.arrows.length; ++i)
                {
                    var begin = Data.arrows[i].begin;
                    var end = Data.arrows[i].end;
                    var beginParentName = begin.parent.name.replace(/[<>:,&.'"\/\\|?*]+/g, '');  //& < > " ' xml special characters
                    var endParentName = end.parent.name.replace(/[<>:,&.'"\/\\|?*]+/g, '');  //& < > " ' xml special characters
                    var beginName = begin.name.replace(/[<>:,&.'"\/\\|?*]+/g, '');  //& < > " ' xml special characters
                    var endName = end.name.replace(/[<>:,&.'"\/\\|?*]+/g, '');  //& < > " ' xml special characters
                ...
                {
                    var begin = Data.inhibitions[i].begin;
                    var end = Data.inhibitions[i].end;
                    var beginParentName = begin.parent.name.replace(/[<>:,&.'"\/\\|?*]+/g, '');  //& < > " ' xml special characters
                    var endParentName = end.parent.name.replace(/[<>:,&.'"\/\\|?*]+/g, '');  //& < > " ' xml special characters
                    var beginName = begin.name.replace(/[<>:,&.'"\/\\|?*]+/g, '');  //& < > " ' xml special characters
                    var endName = end.name.replace(/[<>:,&.'"\/\\|?*]+/g, '');  //& < > " ' xml special characters
